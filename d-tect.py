...
def dtect():
	...
	print("")
	print(" D-TECT - Modern Web Pentest'i")
	print(" Author: Shawar Khan - ( https://shawarkhan.com )")
	print("Türkçe edit: delosemre / blinkcursor ( http://blinkcursor.org )")
	print("")
	def menu():
		global filedetector,wpenumerator,subdomainscan,portscan,wpscan,xssscanner,wpbackupscan,sqliscanner
		print(" -- "+boldwhite+"Menu"+reset+" -- \n \n  1. 	"+boldwhite+"WordPress Kullanıcı Adı Sayıcısı"+reset+"   \n  2. 	"+boldwhite+"Hassas Dosya Dedektörü"+reset+"        \n  3. 	"+boldwhite+"Alt Alan Tarayıcı"+reset+"\n  4. 	"+boldwhite+"Port Tarayıcı"+reset+"        \n  5. 	"+boldwhite+"Wordpress Tarayıcı\n"+reset+"  6. 	"+boldwhite+"Siteler Arası Komut Dosyası Çalıştırma [XSS] Tarayıcı\n"+reset+"  7.    "+boldwhite+"Wordpress Yedekleme Kapmak\n"+reset+"  8.    "+boldwhite+"SQL Enjeksiyon [SQLI] Tarayıcı\n"+reset)
		option = raw_input("[+] Seçeneği Seç\n    > ")
		if option == "1":
			wpenumerator = "on"
		elif option == "2":
			filedetector = "on"
	...
	def cloudflare():
		...
		if "Erişimi kısıtlamak için CloudFlare kullandı</title>" in pagesource:
	...
	def alive():
		try:
			...
			print("[+] Cloudflare'ın erişimi engelleyip engellemediğini kontrol ediliyor...")
			cloudflare()
			redirectcheck()
		except(IOError):
			print("[!] "+red+"Siteye bağlanmada hata oluştu! Site belki çökmüştür."+reset)
			again()
	def responseheadercheck():
		...
		for i in page.headers:
			if i.lower() in str(headers).lower():
				...
				headersfound.append(structure)
		if cj == 0:
			caution.append("[!]"+red+" X-Frame-Options başlığı eksik\n"+reset+"[!] "+red+"Sayfaya karşı savunmasız olabilir "+boldred+"Jacking'i tıklayın\n"+reset+"[!] "+boldred+page.geturl()+reset+"\n[i] ClickJacking Hakkında: [ "+green+"https://www.owasp.org/index.php/Clickjacking"+reset+" ]")
		print("[+] İlginç Başlıklar Bulundu:")
		for i in headersfound:
			print(" | %s")%(i)
		if len(interesting) != 0:
			print("\n[i] Başlıklardaki bilgiler:")
	...
	def parameterarrange(payload):
		...
		for n in parameters:
			try:
				print("Kontrol etme '%s' parameter")%n[0]
				...
				pass
	def SQLIscan(site):
		print("[+] [ SQLI ] Tarayıcı Başlatıldı...\n")
		vuln = []
		payloads = {
					'2':'"',
					'1':'\''
		...
		errors = {
					'MySQL':'SQL sözdiziminde bir hata var;',
					'Oracle':'SQL komutu düzgün şekilde bitmedi',
					'MSSQL':'Karakter dizesinden sonra kapalı olan tırnak işareti',
					'PostgreSQL':'Veya yakınında sözdizimi hatası'
		}
		...
		for n in parameters:
				...
				for i in payloads:
					...
					for i in errors:
						if errors[i] in source:#htmlcode[0].contents[0]:
							dbfound = " | Arka Uç Veritabanı: "+green+str(i)+reset
							found = 1
							break
					if found != 1:
						break
			...
			if found == 1:
				print("[!] "+red+"SQL Enjeksiyon Güvenlik Açığı Buluntu!"+reset)
				print dbfound
				vuln.append("'"+n[0]+"'")
				found = 0
		if len(vuln) != 0:
			print(" | Güvenlik Açığı Parametresi/s:"),
			for i in vuln:
				print(i),
	...
	def XSSscan(site):
		...
		for n in parameters:
			...
			if found == 1:
				...
				found = 0
		if len(vuln) != 0:
			print("[!] "+red+"Güvenlik Açığı Parametresi/s:"+reset),
			for i in vuln:
				print(boldred+i+reset),
		else:
			print("[!] Not Vulnerable")
	def portscanner():
		print("[i] Syntax	:	fonksiyon")
  		print("    23,80,120	:	Spesifik Limanlar, örneğin Tarama Portu 23,80 ve 120'yi tarar.")
  		print("    23-80	:	Bir Port Aralığı Tarama, ör. Tarama Portu 23'den 80'e")
  		print("    23   	:	Tek bir bağlantı noktasını tarar, ör., Bağlantı Noktaları Tarar 23")
  		print("    all  	:	Tüm bağlantı noktalarını 20'den 5000'e tarar")
  		print(" ")
  		portoption = raw_input("[+] Aralık veya Bağlantı Noktasını Girin:\n    > ")
  		wasmultiple		 = 		0
  		wasrange		 = 		0
  		wasone			 =		0
  		if ',' in portoption:
  			...
  			print("[+] Yanlış Söz Dizimi!")
  			again()
		print("[+] Taranıyor %s Port/s hedefte: %s")%(len(totallength),ip)
		ports = 5000
		found = 1
		protocolname = 'tcp'
		progress = 20
		...
		for i in loopcondition:
			...
			portconnect.close()
		if found == 1:
			print("\n | "+red+"Açık Bağlantı Noktası Bulunamadı!"+reset)
	def subdomainscanner():

		import sys
		print("\n[+] Alt Alan Tarayıcı Başlat!")
		...
		for i in wordlist:
			...
			try:
				...
				if (s):
					...
					if s == '127.0.0.1':
						print("[!] "+red+"Alt alan, aşağıdakilere karşı savunmasızdır "+boldred+"Aynı Site Komut Dosyası Oluşturma! "+reset+"\n[!] Aynı Site Komut Dosyası Hakkında:\n[!] [ "+green+"https://www.acunetix.com/vulnerabilities/web/same-site-scripting"+reset+" ] ")
					print('')
	...
	def enumform(listofIDs,listofnames):
		...
		if lengthofnames < 12:
			lengthofnames = 12
		print "[i] "+green+"Aşağıdaki Kullanıcı Bulundu/s:"+reset
		print "\t+-"+'-'.center(6, '-')+'-+-'+'-'.center(lengthofnames, '-')+"-+"
		print "\t| "+'ID/s'.center(6, ' ')+' | '+'Kullanıcı adı/s'.center(lengthofnames, ' ')+" |"
		print "\t+-"+'-'.center(6, '-')+'-+-'+'-'.center(lengthofnames, '-')+"-+"
	...
	def wpbackupscanner():
		...
		print("[+] Wordpress Yedeklemelerini Arama...")
		print("[?] Not: Atlamak için CTRL + C tuşlarına basın.\n  ")
		progress = 0
		backup = []
		backupurl = []
		try:
			for i in backups:
				progress += 1
				sys.stdout.write("\r[+]İlerleme %i / %s ..."% (progress,len(backups)))
				sys.stdout.flush()
				sock(i)
				if page.getcode() == 200:
					detecting = sock(i,"1")
	...
	def wpenumeration():
		...
		url = page.geturl()
		if page.geturl() != host:
			print("[i] Uzak ana makine'"+str(url)+"' \n    Yönlendirmeyi takiben...")
			host = page.geturl()
		print("\n[+] Scan Started : "+lightgreen+"%s"+reset) % time.strftime("%c")
		print "[+] Numaralandırma Kullanıcı Adları ..."
		T = 33
		found = 0
		listofusernames = []
		listofids = []
		...
		if found == 0:
			print("[+] "+red+"Kullanıcı adı bulunamadı"+reset)
		else:
			enumform(listofids,listofusernames)
		print("[+] Numaralandırma Tamamlandı.")
		print("[+] Tarama Bitti : "+lightgreen+"%s"+reset) % time.strftime("%c")
	def wpscanner():
		print("  \n[+] Wordpress'i Algılama")
		wp = 0
		i = 'wp-admin/'
		sock(i)
		if "wp-login.php?redirect_to" in page.geturl():
			wp = 1
			print(green+"[i] "+green+"Wordpress Algılandı!"+reset)
			if wpenumeration == "on":
				wpenumeration()
			else:
		...
		if wp == 0:
			...
			if page.getcode() == 200 and "" in page.read():
				print("[!] "+green+"Wordpress Algılandı!"+reset)
				wp = 1
				if wpenumeration == "on":
					wpenumeration()
				else:
					...
					wpenumeration()
		if wp == 0:
			print("[!] "+red+"Hiçbir Wordpress Algılanamadı"+reset)
	def redirectcheck():
		global redirect,host
		redirect = 0
		print("[+] Yeniden Yönlendirmeyi Denetleme")
		page = urllib.urlopen(host)
		url = page.geturl()
		if page.geturl() != host:
			option = raw_input("[i] "+boldgrey+"Ana makine şu kişilere yönlendirir:"+str(url)+reset+" \n    Bunu varsayılan Ana Bilgisayar olarak ayarla mı? [Y/N]:\n    > ")
			if option.lower() == "y":
				host = page.geturl()
				redirect = 1
		else:
			print("[+] URL yeniden yönlendirme yapmıyor")
	def again():
		...
		sqliscanner		=		"off"
		# -- Swiches Reset --
		inp = raw_input("\n[+] [E]Çık veya başlat [A]tekrar? (e/a)").lower()
		if inp == 'a':
			dtect()
		elif inp == 'e':
			exit()
		...
		if wpenumerator == "on":
			...
			if wp == 0:
				...
				if page.getcode() == 200 and "" in page.read():
					...
					wpenumeration()
			if wp == 0:
				print("[!] "+red+"Hiçbir Wordpress Algılandı"+reset)
		if wpscan == "on":
			wpscanner()
		if subdomainscan == "on":
			subdomainscanner()
